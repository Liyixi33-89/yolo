...
@app.post("/api/baidu-free/speech")
...
@app.post("/api/baidu-free/image-search")
async def baidu_free_image_search(request: BaiduFreeImageSearchRequest):
    """
    百度免费图像搜索 API
    支持：相同图片搜索、相似图片搜索、商品图片搜索、绘本图片搜索、面料图片搜索
    """
    try:
        ...
        url = search_url_map.get(request.search_type, search_url_map["same"])
...
@app.post("/api/baidu-free/image-search/add")
async def baidu_free_image_add(request: BaiduFreeImageSearchRequest):
    ...
    try:
        ...
        url = add_url_map.get(request.search_type, add_url_map["same"])
        url = f"{url}?access_token={access_token}"
        
        headers = {"Content-Type": "application/x-www-form-urlencoded"}
        data = {
            "image": image_base64,
            "brief": request.brief or "无描述"
        }
...
@app.post("/api/baidu-free/image-search/delete")
async def baidu_free_image_delete(request: Request):
    ...
    try:
        ...
        url = delete_url_map.get(search_type, delete_url_map["same"])
        url = f"{url}?access_token={access_token}"
